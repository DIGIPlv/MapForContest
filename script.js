...
function putPins ()
{
 ...
 for(let i = 0; i < mapInfo.length; i++)
 {
   ...
   let mapOption = '';
   switch(parseInt(document.getElementById('option').innerHTML))
   {
     case 1:
      mapOption = 5;
      break;
   ...
   if (mapInfo[i][mapOption] != 0)
   {
     ...
     option.innerHTML = (i + 1) + '. ' + mapInfo[i][0];
     option.value = mapInfo[i][0];
...
function createPopup(id)
{
 ...
 if(mapInfo[id][mapOption] != 0)
 {
   let contactWrap = document.createElement('div');
   contactWrap.className = 'wrap-btn mt_40';
   let contact = document.createElement('button');
   contact.className = 'olimpiade__btn';
   contact.innerHTML = 'Izvēlēties';
   contact.addEventListener('click', (e) =>
   {
     let allOptions = document.getElementsByTagName('option');
     document.getElementById('item').value = mapInfo[id][8];
     let j = 0;
     while(j < allOptions.length)
     {
       if(allOptions[j].innerHTML.includes(mapInfo[id][0]))
       {
 ...
 else
 {
   ...
   notAvailable.className = 'map__txt-not-available';
   notAvailable.innerHTML = 'Šis objektu ir jau aizņemts!';
   mapModalContent.append(notAvailable);
 }
 // Close btn
...
// Showing selected pin
  ...
  {
    if(e.target.value == mapInfo[a][0])
    {
      selectedPin = a;
      document.getElementById('item').value = mapInfo[a][8];
      break;
    }
    a++;
  }
